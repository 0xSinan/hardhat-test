...
//       ({ deployedContract, owner, addr1, addr2 } = await loadFixture(
//         deployBankFixture
//       ));
//     });
...
//       await expect(
//         deployedContract.connect(addr1).deposit({ value: weiQuantity })
//       )
...
//       await expect(
//         deployedContract.deposit({ value: weiQuantity })
...
//       let weiQuantity = hre.ethers.parseEther("0.19");
//       await expect(
//         deployedContract.connect(owner).deposit({ value: weiQuantity })
//       )
//         .to.emit(deployedContract, "Deposit")
//         .withArgs(owner.address, weiQuantity);

//       let balanceContract = await hre.ethers.provider.getBalance(
...
//       await expect(deployedContract.withdraw(weiQuantity)).to.be.revertedWith(
