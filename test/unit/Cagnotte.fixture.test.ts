...
describe("Test Cagnotte", function () {
  ...
  async function deployCagnotteEthersGivenGoalNotReachedFixture() {
    const { cagnotte, goal, owner, addr1, addr2, addr3 } = await loadFixture(
      deployCagnotteFixture
    );
  ...
  describe("Initialization", function () {
    it("should NOT deploy the contract if the goal is 0", async function () {
      ...
      await expect(
        ethers.deployContract("Cagnotte", [goal])
  ...
  describe("Deposit", function () {
    beforeEach(async () => {
      ({ cagnotte, goal, owner, addr1, addr2, addr3 } = await loadFixture(
        deployCagnotteFixture
      ));
    });
    ...
    it("should NOT deposit Ethers on the Cagnotte contract if not enough funds provided", async function () {
      ...
      await expect(cagnotte.deposit({ value: weiQuantity })).to.be.rejectedWith(
  ...
  describe("Withdraw", function () {
    ...
    it("should withdraw if the goal is reached and if it is the owner who tries to withdraw", async function () {
      ...
      let ownerBalanceBeforeWithdraw = await ethers.provider.getBalance(
        owner.address
      );
      let contractBalance = await ethers.provider.getBalance(cagnotte.target);

      await expect(cagnotte.withdraw())
        .to.emit(cagnotte, "Withdrawal")
        .withArgs(owner, contractBalance);

      let ownerBalanceAfterWithdraw = await ethers.provider.getBalance(
        owner.address
      );
      expect(ownerBalanceAfterWithdraw).to.be.greaterThan(
